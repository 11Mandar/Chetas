...
# Ensure TLS protocols are available
...
if ($sent) {
    ...
    Write-Host "    Make sure Chetas server is running and reachable." -ForegroundColor DarkYellow
...
while ($true) {
    ...
    foreach ($ev in $events) {
        ...
        if (-not $sentIds.Contains($key)) {
            ...
            if ($success) {
                Write-Host "[$timeStr] [#$counter] INGESTED: $localIp -> $ChetasServerIp | $prov (Event $id) [$lvl]" -ForegroundColor Green
            } else {
